...
struct otri {
  triangle *tri;
  int64_t orient;                                         /* Ranges from 0 to 2. */
};
...
struct osub {
  subseg *ss;
  int64_t ssorient;                                       /* Ranges from 0 to 1. */
};
...
struct event {
  ...
  int64_t heapposition;              /* Marks this event's position in the heap. */
};
...
struct memorypool {
  ...
  int64_t alignbytes;
  int64_t itembytes;
  int64_t itemsperblock;
  int64_t itemsfirstblock;
  int64_t items, maxitems;
  int64_t unallocateditems;
  int64_t pathitemsleft;
};
...
int64_t randomseed;                     /* Current random number seed. */
...
/*   ordered from 4095 (highest priority) to 0 (lowest priority).            */
  ...
  int64_t nextnonemptyq[4096];
  int64_t firstnonemptyq;
...
/* Other variables. */
  ...
  int64_t invertices;                               /* Number of input vertices. */
  int64_t inelements;                              /* Number of input triangles. */
  int64_t insegments;                               /* Number of input segments. */
  int64_t holes;                                       /* Number of input holes. */
  int64_t regions;                                   /* Number of input regions. */
  int64_t undeads;    /* Number of input vertices that don't appear in the mesh. */
  int64_t edges;                                     /* Number of output edges. */
  int64_t mesh_dim;                                /* Dimension (ought to be 2). */
  int64_t nextras;                           /* Number of attributes per vertex. */
  int64_t eextras;                         /* Number of attributes per triangle. */
  int64_t hullsize;                          /* Number of edges in convex hull. */
  int64_t steinerleft;                 /* Number of Steiner points not yet used. */
  int64_t vertexmarkindex;         /* Index to find boundary marker of a vertex. */
  int64_t vertex2triindex;     /* Index to find a triangle adjacent to a vertex. */
  int64_t highorderindex;  /* Index to find extra nodes for high-order elements. */
  int64_t elemattribindex;            /* Index to find attributes of a triangle. */
  int64_t areaboundindex;             /* Index to find area bound of a triangle. */
  int64_t checksegments;         /* Are there segments in the triangulation yet? */
  int64_t checkquality;                  /* Has quality triangulation begun yet? */
  int64_t readnodefile;                           /* Has a .node file been read? */
  int64_t samples;              /* Number of random samples for point location. */

  int64_t incirclecount;                 /* Number of incircle tests performed. */
  int64_t counterclockcount;     /* Number of counterclockwise tests performed. */
  int64_t orient3dcount;           /* Number of 3D orientation tests performed. */
  int64_t hyperbolacount;      /* Number of right-of-hyperbola tests performed. */
  int64_t circumcentercount;  /* Number of circumcenter calculations performed. */
  int64_t circletopcount;       /* Number of circle top calculations performed. */
...
/* Read the instructions to find out the meaning of these switches.          */
  ...
  int64_t poly, refine, quality, vararea, fixedarea, usertest;
  int64_t regionattrib, convex, weighted, jettison;
  int64_t firstnumber;
  int64_t edgesout, voronoi, neighbors, geomview;
  int64_t nobound, nopolywritten, nonodewritten, noelewritten, noiterationnum;
  int64_t noholes, noexact, conformdel;
  int64_t incremental, sweepline, dwyer;
  int64_t splitseg;
  int64_t docheck;
  int64_t quiet, verbose;
  int64_t usesegments;
  int64_t order;
  int64_t nobisect;
  int64_t steiner;
...
int64_t plus1mod3[3] = {1, 2, 0};
int64_t minus1mod3[3] = {2, 0, 1};
...
#define decode(ptr, otri)                                                     \
  (otri).orient = (int64_t) ((int64_t) (ptr) & (int64_t) 3l);         \
  (otri).tri = (triangle *)                                                   \
                  ((int64_t) (ptr) ^ (int64_t) (otri).orient)
...
#define encode(otri)                                                          \
  (triangle) ((int64_t) (otri).tri | (int64_t) (otri).orient)
...
#define infected(otri)                                                        \
  (((int64_t) (otri).tri[6] & (int64_t) 2l) != 0l)
...
#define sdecode(sptr, osub)                                                   \
  (osub).ssorient = (int64_t) ((int64_t) (sptr) & (int64_t) 1l);      \
  (osub).ss = (subseg *)                                                      \
              ((int64_t) (sptr) & ~ (int64_t) 3l)
...
#define sencode(osub)                                                         \
  (subseg) ((int64_t) (osub).ss | (int64_t) (osub).ssorient)
...
#define mark(osub)  (* (int64_t *) ((osub).ss + 8))

#define setmark(osub, value)                                                  \
  * (int64_t *) ((osub).ss + 8) = value
...
#define vertexmark(vx)  ((int64_t *) (vx))[m->vertexmarkindex]

#define setvertexmark(vx, value)                                              \
  ((int64_t *) (vx))[m->vertexmarkindex] = value

#define vertextype(vx)  ((int64_t *) (vx))[m->vertexmarkindex + 1]

#define setvertextype(vx, value)                                              \
  ((int64_t *) (vx))[m->vertexmarkindex + 1] = value
...
int64_t triunsuitable();

#else /* not EXTERNAL_TEST */

#ifdef ANSI_DECLARATORS
int64_t triunsuitable(vertex triorg, vertex tridest, vertex triapex, REAL area)
#else /* not ANSI_DECLARATORS */
int64_t triunsuitable(triorg, tridest, triapex, area)
...

{
  ...
  memptr = (VOID *) malloc((int64_t) size);
  if (memptr == (VOID *) NULL) {
...
"    Triangle calculates the intersection points for you and adds them to\n");
  printf(
"    the triangulation--as int64_t as your machine's floating-point precision\n");
  printf(
...
"  row of evenly spaced, segment-connected vertices?  Have you simply\n");
  printf(
"  defined one int64_t segment connecting the leftmost vertex to the rightmost\n"
);
  printf(
...
#ifdef ANSI_DECLARATORS
void parsecommandline(int64_t argc, char **argv, struct behavior *b)
#else /* not ANSI_DECLARATORS */
    ...
    if (argv[i][0] == '-') {
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'S') {
          ...
          while ((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) {
            j++;
            b->steiner = b->steiner * 10 + (int64_t) (argv[i][j] - '0');
          }
        }
#endif /* not CDT_ONLY */
#ifndef REDUCED
  ...
  if (increment > 0) {
    ...
    do {
      if ((workstring[j] >= '0') && (workstring[j] <= '9')) {
        meshnumber = meshnumber * 10 + (int64_t) (workstring[j] - '0');
      } else {
        increment = 0;
      }
      j++;
...

{
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 1) % 3 + 3, (int64_t) printvertex,
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           (t->orient + 2) % 3 + 3, (int64_t) printvertex,
           printvertex[0], printvertex[1]);
  apex(*t, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Apex  [%d] = NULL\n", t->orient + 3);
    ...
    printf("    Apex  [%d] = x%lx  (%.12g, %.12g)\n",
           t->orient + 3, (int64_t) printvertex,
           printvertex[0], printvertex[1]);
...

{
  ...
  printf("subsegment x%lx with orientation %d and mark %d:\n",
         (int64_t) s->ss, s->ssorient, mark(*s));
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Origin[%d] = x%lx  (%.12g, %.12g)\n",
           2 + s->ssorient, (int64_t) printvertex,
           printvertex[0], printvertex[1]);
  sdest(*s, printvertex);
  if (printvertex == (vertex) NULL)
    printf("    Dest  [%d] = NULL\n", 3 - s->ssorient);
    ...
    printf("    Dest  [%d] = x%lx  (%.12g, %.12g)\n",
           3 - s->ssorient, (int64_t) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment origin[%d] = x%lx  (%.12g, %.12g)\n",
           4 + s->ssorient, (int64_t) printvertex,
           printvertex[0], printvertex[1]);
  ...
  if (printvertex == (vertex) NULL)
    ...
    printf("    Segment dest  [%d] = x%lx  (%.12g, %.12g)\n",
           5 - s->ssorient, (int64_t) printvertex,
           printvertex[0], printvertex[1]);
}
...

{
  ...
  alignptr = (int64_t) (pool->nowblock + 1);
  /* Align the item on an `alignbytes'-byte boundary. */
  pool->nextitem = (VOID *)
    (alignptr + (int64_t) pool->alignbytes -
     (alignptr % (int64_t) pool->alignbytes));
...
#ifdef ANSI_DECLARATORS
void poolinit(struct memorypool *pool, int64_t bytecount, int64_t itemcount,
              int64_t firstitemcount, int64_t alignment)
#else /* not ANSI_DECLARATORS */
void poolinit(pool, bytecount, itemcount, firstitemcount, alignment)
struct memorypool *pool;
int64_t bytecount;
int64_t itemcount;
int64_t firstitemcount;
int64_t alignment;
#endif /* not ANSI_DECLARATORS */

{
  ...
  pool->firstblock = (VOID **)
    trimalloc(pool->itemsfirstblock * pool->itembytes + (int64_t) sizeof(VOID *) +
              pool->alignbytes);
...

{
  ...
  alignptr = (int64_t) (pool->pathblock + 1);
...
#ifdef ANSI_DECLARATORS
void dummyinit(struct mesh *m, struct behavior *b, int64_t trianglebytes,
               int64_t subsegbytes)
#else /* not ANSI_DECLARATORS */
void dummyinit(m, b, trianglebytes, subsegbytes)
struct mesh *m;
struct behavior *b;
int64_t trianglebytes;
int64_t subsegbytes;
#endif /* not ANSI_DECLARATORS */

{
  ...
  alignptr = (int64_t) m->dummytribase;
  m->dummytri = (triangle *)
    (alignptr + (int64_t) m->triangles.alignbytes -
     (alignptr % (int64_t) m->triangles.alignbytes));
  /* Initialize the three adjoining triangles to be "outer space."  These  */
  /*   will eventually be changed by various bonding operations, but their */
  /*   values don't really matter, as int64_t as they can legally be          */
  /*   dereferenced.                                                       */
  ...
  if (b->usesegments) {
    ...
    alignptr = (int64_t) m->dummysubbase;
    m->dummysub = (subseg *)
      (alignptr + (int64_t) m->subsegs.alignbytes -
       (alignptr % (int64_t) m->subsegs.alignbytes));
    /* Initialize the two adjoining subsegments to be the omnipresent      */
    /*   subsegment.  These will eventually be changed by various bonding  */
    /*   operations, but their values don't really matter, as int64_t as they */
...

{
  ...
  /*   a sizeof(int64_t)-byte address.                                          */
  m->vertexmarkindex = ((m->mesh_dim + m->nextras) * sizeof(REAL) +
                        sizeof(int64_t) - 1) /
                       sizeof(int64_t);
  vertexsize = (m->vertexmarkindex + 2) * sizeof(int64_t);
  if (b->poly) {
...

{
  ...
  if ((b->voronoi || b->neighbors) &&
      (trisize < 6 * sizeof(triangle) + sizeof(int64_t))) {
    trisize = 6 * sizeof(triangle) + sizeof(int64_t);
  }
  ...
  if (b->usesegments) {
    ...
    poolinit(&m->subsegs, 8 * sizeof(triangle) + sizeof(int64_t),
...
#ifdef ANSI_DECLARATORS
vertex getvertex(struct mesh *m, struct behavior *b, int64_t number)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  alignptr = (int64_t) (getblock + 1);
  foundvertex = (char *) (alignptr + (int64_t) m->vertices.alignbytes -
                          (alignptr % (int64_t) m->vertices.alignbytes));
...
#ifdef ANSI_DECLARATORS
int64_t fast_expansion_sum_zeroelim(int64_t elen, REAL *e, int64_t flen, REAL *f, REAL *h)
#else /* not ANSI_DECLARATORS */
int64_t fast_expansion_sum_zeroelim(elen, e, flen, f, h)  /* h cannot be e or f. */
int64_t elen;
REAL *e;
int64_t flen;
REAL *f;
...
#ifdef ANSI_DECLARATORS
int64_t scale_expansion_zeroelim(int64_t elen, REAL *e, REAL b, REAL *h)
#else /* not ANSI_DECLARATORS */
int64_t scale_expansion_zeroelim(elen, e, b, h)   /* e and h cannot be the same. */
int64_t elen;
REAL *e;
REAL b;
REAL *h;
...
#ifdef ANSI_DECLARATORS
REAL estimate(int64_t elen, REAL *e)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  INEXACT REAL B3;
  int64_t C1length, C2length, Dlength;
  REAL u[4];
  INEXACT REAL u3;
  INEXACT REAL s1, t1;
  REAL s0, t0;
...

{
  ...
  int64_t axbclen, axxbclen, aybclen, ayybclen, alen;
  REAL bxca[8], bxxca[16], byca[8], byyca[16], bdet[32];
  int64_t bxcalen, bxxcalen, bycalen, byycalen, blen;
  REAL cxab[8], cxxab[16], cyab[8], cyyab[16], cdet[32];
  int64_t cxablen, cxxablen, cyablen, cyyablen, clen;
  REAL abdet[64];
  int64_t ablen;
  ...
  int64_t temp8len, temp16alen, temp16blen, temp16clen;
  int64_t temp32alen, temp32blen, temp48len, temp64len;
  REAL axtbb[8], axtcc[8], aytbb[8], aytcc[8];
  int64_t axtbblen, axtcclen, aytbblen, aytcclen;
  REAL bxtaa[8], bxtcc[8], bytaa[8], bytcc[8];
  int64_t bxtaalen, bxtcclen, bytaalen, bytcclen;
  REAL cxtaa[8], cxtbb[8], cytaa[8], cytbb[8];
  int64_t cxtaalen, cxtbblen, cytaalen, cytbblen;
  REAL axtbc[8], aytbc[8], bxtca[8], bytca[8], cxtab[8], cytab[8];
  int64_t axtbclen, aytbclen, bxtcalen, bytcalen, cxtablen, cytablen;
  REAL axtbct[16], aytbct[16], bxtcat[16], bytcat[16], cxtabt[16], cytabt[16];
  int64_t axtbctlen, aytbctlen, bxtcatlen, bytcatlen, cxtabtlen, cytabtlen;
  REAL axtbctt[8], aytbctt[8], bxtcatt[8];
  REAL bytcatt[8], cxtabtt[8], cytabtt[8];
  int64_t axtbcttlen, aytbcttlen, bxtcattlen, bytcattlen, cxtabttlen, cytabttlen;
  REAL abt[8], bct[8], cat[8];
  int64_t abtlen, bctlen, catlen;
  REAL abtt[4], bctt[4], catt[4];
  int64_t abttlen, bcttlen, cattlen;
...

{
  ...
  int64_t at_blen, at_clen, bt_clen, bt_alen, ct_alen, ct_blen;
...
void findcircumcenter(struct mesh *m, struct behavior *b,
                      ...
                      vertex circumcenter, REAL *xi, REAL *eta, int64_t offcenter)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
int64_t randomnation(int64_t choices)
#else /* not ANSI_DECLARATORS */
int64_t randomnation(choices)
int64_t choices;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
int64_t checkseg4encroach(struct mesh *m, struct behavior *b,
                      struct osub *testsubseg)
#else /* not ANSI_DECLARATORS */
int64_t checkseg4encroach(m, b, testsubseg)
struct mesh *m;
struct behavior *b;
struct osub *testsubseg;
#endif /* not ANSI_DECLARATORS */
...
/*  int64_t as the search is begun from the triangle in question.               */
/*                                                                           */
/*****************************************************************************/
...

{
  ...
  REAL ahead;
  int64_t samplesperblock, totalsamplesleft, samplesleft;
  int64_t population, totalpopulation;
  triangle ptr;                         /* Temporary variable used by sym(). */
  ...
  while (totalsamplesleft > 0) {
    ...
    alignptr = (int64_t) (sampleblock + 1);
    firsttri = (char *) (alignptr +
                         (int64_t) m->triangles.alignbytes -
                         (alignptr %
                          (int64_t) m->triangles.alignbytes));
    ...
    do {
      sampletri.tri = (triangle *) (firsttri +
                                    (randomnation((int64_t) population) *
...
int64_t subsegmark;                            /* Marker for the new subsegment. */
#endif /* not ANSI_DECLARATORS */
...
enum insertvertexresult insertvertex(struct mesh *m, struct behavior *b,
                                     ...
                                     int64_t segmentflaws, int64_t triflaws)
#else /* not ANSI_DECLARATORS */
        ...
        if (doflip) {
          ...
          if (b->vararea) {
            if ((areabound(top) <= 0.0) || (areabound(horiz) <= 0.0)) {
              ...
              /*   int64_t, int64_t way from their original location due to flips. */
...
/*  int64_t as the fan remains connected to them.                               */
/*                                                                           */
...
/*  there's a lot of int64_t segments that each cut many triangles.  I ought to */
...
void triangulatepolygon(struct mesh *m, struct behavior *b,
                        ...
                        int64_t edgecount, int64_t doflip, int64_t triflaws)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void vertexsort(vertex *sortarray, int64_t arraysize)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  pivot = (int64_t) randomnation((int64_t) arraysize);
...
#ifdef ANSI_DECLARATORS
void vertexmedian(vertex *sortarray, int64_t arraysize, int64_t median, int64_t axis)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void alternateaxes(vertex *sortarray, int64_t arraysize, int64_t axis)
#else /* not ANSI_DECLARATORS */
...
void mergehulls(struct mesh *m, struct behavior *b, struct otri *farleft,
                ...
                struct otri *farright, int64_t axis)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  int64_t changemade;
  int64_t badedge;
  int64_t leftfinished, rightfinished;
  triangle ptr;                         /* Temporary variable used by sym(). */
...
#ifdef ANSI_DECLARATORS
int64_t removeghosts(struct mesh *m, struct behavior *b, struct otri *startghost)
#else /* not ANSI_DECLARATORS */
int64_t removeghosts(m, b, startghost)
struct mesh *m;
struct behavior *b;
...
#ifdef ANSI_DECLARATORS
int64_t divconqdelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
int64_t divconqdelaunay(m, b)
struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
  ...
  sortarray = (vertex *) trimalloc(m->invertices * (int64_t) sizeof(vertex));
...
#ifdef ANSI_DECLARATORS
int64_t removebox(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
int64_t removebox(m, b)
struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
int64_t incrementaldelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
int64_t incrementaldelaunay(m, b)
struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void eventheapinsert(struct event **heap, int64_t heapsize, struct event *newevent)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void eventheapify(struct event **heap, int64_t heapsize, int64_t eventnum)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void eventheapdelete(struct event **heap, int64_t heapsize, int64_t eventnum)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  *eventheap = (struct event **) trimalloc(maxevents *
                                           (int64_t) sizeof(struct event *));
  *events = (struct event *) trimalloc(maxevents * (int64_t) sizeof(struct event));
  traversalinit(&m->vertices);
  for (i = 0; i < m->invertices; i++) {
...
#ifdef ANSI_DECLARATORS
int64_t rightofhyperbola(struct mesh *m, struct otri *fronttri, vertex newsite)
#else /* not ANSI_DECLARATORS */
int64_t rightofhyperbola(m, fronttri, newsite)
struct mesh *m;
...
void check4deadevent(struct otri *checktri, struct event **freeevents,
                     struct event **eventheap, int64_t *heapsize)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  vertex checkvertex;
  int64_t rightofroot, rightofchild;
...
struct splaynode *frontlocate(struct mesh *m, struct splaynode *splayroot,
                              ...
                              struct otri *searchtri, int64_t *farright)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
int64_t sweeplinedelaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
int64_t sweeplinedelaunay(m, b)
struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */

{
  ...
  int64_t heapsize;
  int64_t check4events, farrightflag;
  triangle ptr;   /* Temporary variable used by sym(), onext(), and oprev(). */
...
#ifdef ANSI_DECLARATORS
int64_t delaunay(struct mesh *m, struct behavior *b)
#else /* not ANSI_DECLARATORS */
int64_t delaunay(m, b)
struct mesh *m;
struct behavior *b;
#endif /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
int64_t reconstruct(struct mesh *m, struct behavior *b, int64_t *trianglelist,
                REAL *triangleattriblist, REAL *trianglearealist,
                int64_t elements, int64_t corners, int64_t attribs,
                int64_t *segmentlist,int64_t *segmentmarkerlist, int64_t numberofsegments)
#else /* not ANSI_DECLARATORS */
int64_t reconstruct(m, b, trianglelist, triangleattriblist, trianglearealist,
                elements, corners, attribs, segmentlist, segmentmarkerlist,
                numberofsegments)
struct mesh *m;
struct behavior *b;
int64_t *trianglelist;
...
#ifdef ANSI_DECLARATORS
int64_t reconstruct(struct mesh *m, struct behavior *b, char *elefilename,
                 char *areafilename, char *polyfilename, FILE *polyfile)
#else /* not ANSI_DECLARATORS */
int64_t reconstruct(m, b, elefilename, areafilename, polyfilename, polyfile)
struct mesh *m;
struct behavior *b;
char *elefilename;
char *areafilename;
...

{
  ...
  int64_t corner[3];
  int64_t end[2];
  int64_t killvertexindex;
  int64_t incorners;
  int64_t segmentmarkers;
  int64_t boundmarker;
  int64_t aroundvertex;
  int64_t hullsize;
  int64_t notfound;
  int64_t elementnumber, segmentnumber;
  int64_t i, j;
  triangle ptr;                         /* Temporary variable used by sym(). */
  ...
  m->inelements = (int64_t) strtol(stringptr, &stringptr, 0);
  stringptr = findfield(stringptr);
  if (*stringptr == '\0') {
    incorners = 3;
  } else {
    incorners = (int64_t) strtol(stringptr, &stringptr, 0);
    if (incorners < 3) {
  ...
  if (*stringptr == '\0') {
    ...
    m->eextras = (int64_t) strtol(stringptr, &stringptr, 0);
  }
#endif /* not TRILIBRARY */
  ...
  if (b->poly) {
    ...
    segmentmarkers = segmentmarkerlist != (int64_t *) NULL;
#else /* not TRILIBRARY */
    ...
    m->insegments = (int64_t) strtol(stringptr, &stringptr, 0);
    stringptr = findfield(stringptr);
    if (*stringptr != '\0') {
      segmentmarkers = (int64_t) strtol(stringptr, &stringptr, 0);
    }
#endif /* not TRILIBRARY */
  ...
  if (b->vararea) {
    ...
    areaelements = (int64_t) strtol(stringptr, &stringptr, 0);
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        corner[j] = (int64_t) strtol(stringptr, &stringptr, 0);
        if ((corner[j] < b->firstnumber) ||
            (corner[j] >= b->firstnumber + m->invertices)) {
          printf("Error:  Triangle %ld has an invalid vertex index.\n",
                 elementnumber);
    ...
    for (j = 3; j < incorners; j++) {
      ...
      stringptr = findfield(stringptr);
      if (*stringptr != '\0') {
        killvertexindex = (int64_t) strtol(stringptr, &stringptr, 0);
#endif /* not TRILIBRARY */
  ...
  if (b->poly) {
    ...
    while (subsegloop.ss != (subseg *) NULL) {
      ...
      if (*stringptr == '\0') {
        ...
        end[0] = (int64_t) strtol(stringptr, &stringptr, 0);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        end[1] = (int64_t) strtol(stringptr, &stringptr, 0);
      }
      if (segmentmarkers) {
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
          ...
          boundmarker = (int64_t) strtol(stringptr, &stringptr, 0);
        }
      }
#endif /* not TRILIBRARY */
      for (j = 0; j < 2; j++) {
...
#ifdef ANSI_DECLARATORS
int64_t scoutsegment(struct mesh *m, struct behavior *b, struct otri *searchtri,
                 vertex endpoint2, int64_t newmark)
#else /* not ANSI_DECLARATORS */
int64_t scoutsegment(m, b, searchtri, endpoint2, newmark)
struct mesh *m;
struct behavior *b;
struct otri *searchtri;
vertex endpoint2;
...
void conformingedge(struct mesh *m, struct behavior *b,
                    vertex endpoint1, vertex endpoint2, int64_t newmark)
#else /* not ANSI_DECLARATORS */
...
void delaunayfixup(struct mesh *m, struct behavior *b,
                   struct otri *fixuptri, int64_t leftside)
#else /* not ANSI_DECLARATORS */
...
void constrainededge(struct mesh *m, struct behavior *b,
                     struct otri *starttri, vertex endpoint2, int64_t newmark)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void formskeleton(struct mesh *m, struct behavior *b, int64_t *segmentlist,
                  int64_t *segmentmarkerlist, int64_t numberofsegments)
#else /* not ANSI_DECLARATORS */
  ...
  if (b->poly) {
    ...
    for (i = 0; i < m->insegments; i++) {
      ...
      if (*stringptr == '\0') {
        ...
        end1 = (int64_t) strtol(stringptr, &stringptr, 0);
      }
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        ...
        end2 = (int64_t) strtol(stringptr, &stringptr, 0);
      }
      if (segmentmarkers) {
        stringptr = findfield(stringptr);
        if (*stringptr == '\0') {
...
#ifdef ANSI_DECLARATORS
void carveholes(struct mesh *m, struct behavior *b, REAL *holelist, int64_t holes,
                REAL *regionlist, int64_t regions)
#else /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void splitencsegs(struct mesh *m, struct behavior *b, int64_t triflaws)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  int64_t acuteorg, acuteorg2, acutedest, acutedest2;
  int64_t dummy;
  int64_t i;
  triangle ptr;                     /* Temporary variable used by stpivot(). */
  subseg sptr;                        /* Temporary variable used by snext(). */
...

{
  ...
  if (b->poly) {
    ...
    *polyfile = fopen(polyfilename, "r");
    ...
    stringptr = readline(inputline, *polyfile, polyfilename);
    m->invertices = (int64_t) strtol(stringptr, &stringptr, 0);
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      m->mesh_dim = 2;
    } else {
      m->mesh_dim = (int64_t) strtol(stringptr, &stringptr, 0);
    }
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      m->nextras = 0;
    } else {
      m->nextras = (int64_t) strtol(stringptr, &stringptr, 0);
    }
    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      nodemarkers = 0;
    } else {
      nodemarkers = (int64_t) strtol(stringptr, &stringptr, 0);
    }
    if (m->invertices > 0) {
      infile = *polyfile;
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    if (i == 0) {
      firstnode = (int64_t) strtol(stringptr, &stringptr, 0);
    ...
    if (nodemarkers) {
      ...
      if (*stringptr == '\0') {
        ...
        currentmarker = (int64_t) strtol(stringptr, &stringptr, 0);
...
void transfernodes(struct mesh *m, struct behavior *b, REAL *pointlist,
                   REAL *pointattriblist, int64_t *pointmarkerlist,
                   int64_t numberofpoints, int64_t numberofpointattribs)
#else /* not ANSI_DECLARATORS */
...
int64_t *pointmarkerlist;
int64_t numberofpoints;
int64_t numberofpointattribs;
#endif /* not ANSI_DECLARATORS */

{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    if (pointmarkerlist != (int64_t *) NULL) {
      /* Read a vertex marker. */
...
void readholes(struct mesh *m, struct behavior *b,
               FILE *polyfile, char *polyfilename, REAL **hlist, int64_t *holes,
               REAL **rlist, int64_t *regions)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  /* Read the holes. */
  stringptr = readline(inputline, polyfile, polyfilename);
  *holes = (int64_t) strtol(stringptr, &stringptr, 0);
  if (*holes > 0) {
    holelist = (REAL *) trimalloc(2 * *holes * (int64_t) sizeof(REAL));
    *hlist = holelist;
    for (i = 0; i < 2 * *holes; i += 2) {
      stringptr = readline(inputline, polyfile, polyfilename);
      stringptr = findfield(stringptr);
  ...
  if ((b->regionattrib || b->vararea) && !b->refine) {
    ...
    stringptr = readline(inputline, polyfile, polyfilename);
    *regions = (int64_t) strtol(stringptr, &stringptr, 0);
    if (*regions > 0) {
      regionlist = (REAL *) trimalloc(4 * *regions * (int64_t) sizeof(REAL));
      *rlist = regionlist;
      index = 0;
      for (i = 0; i < *regions; i++) {
        stringptr = readline(inputline, polyfile, polyfilename);
...
#ifdef ANSI_DECLARATORS
void finishfile(FILE *outfile, int64_t argc, char **argv)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*pointlist == (REAL *) NULL) {
    *pointlist = (REAL *) trimalloc((int64_t) (outvertices * 2 * sizeof(REAL)));
  }
  /* Allocate memory for output vertex attributes if necessary. */
  if ((m->nextras > 0) && (*pointattriblist == (REAL *) NULL)) {
    *pointattriblist = (REAL *) trimalloc((int64_t) (outvertices * m->nextras *
                                                 sizeof(REAL)));
  }
  /* Allocate memory for output vertex markers if necessary. */
  if (!b->nobound && (*pointmarkerlist == (int64_t *) NULL)) {
    *pointmarkerlist = (int64_t *) trimalloc((int64_t) (outvertices * sizeof(int64_t)));
  }
  plist = *pointlist;
...
void writeelements(struct mesh *m, struct behavior *b,
                   int64_t **trianglelist, REAL **triangleattriblist)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  if (*trianglelist == (int64_t *) NULL) {
    *trianglelist = (int64_t *) trimalloc((int64_t) (m->triangles.items *
                                             ((b->order + 1) * (b->order + 2) /
                                              2) * sizeof(int64_t)));
  }
  /* Allocate memory for output triangle attributes if necessary. */
  if ((m->eextras > 0) && (*triangleattriblist == (REAL *) NULL)) {
    *triangleattriblist = (REAL *) trimalloc((int64_t) (m->triangles.items *
                                                    m->eextras *
                                                    sizeof(REAL)));
  }
  tlist = *trianglelist;
...
void writepoly(struct mesh *m, struct behavior *b,
               int64_t **segmentlist, int64_t **segmentmarkerlist)
#else /* not ANSI_DECLARATORS */
...
void writepoly(struct mesh *m, struct behavior *b, char *polyfilename,
               REAL *holelist, int64_t holes, REAL *regionlist, int64_t regions,
               int64_t argc, char **argv)
#else /* not ANSI_DECLARATORS */
...

{
...
#else /* not TRILIBRARY */
  FILE *outfile;
  int64_t holenumber, regionnumber;
#endif /* not TRILIBRARY */
  struct osub subsegloop;
  vertex endpoint1, endpoint2;
  int64_t subsegnumber;
  ...
  if (*segmentlist == (int64_t *) NULL) {
    *segmentlist = (int64_t *) trimalloc((int64_t) (m->subsegs.items * 2 *
                                            sizeof(int64_t)));
  }
  /* Allocate memory for output segment markers if necessary. */
  if (!b->nobound && (*segmentmarkerlist == (int64_t *) NULL)) {
    *segmentmarkerlist = (int64_t *) trimalloc((int64_t) (m->subsegs.items *
                                                  sizeof(int64_t)));
  }
  slist = *segmentlist;
...
void writeedges(struct mesh *m, struct behavior *b,
                int64_t **edgelist, int64_t **edgemarkerlist)
#else /* not ANSI_DECLARATORS */
void writeedges(m, b, edgelist, edgemarkerlist)
struct mesh *m;
struct behavior *b;
int64_t **edgelist;
int64_t **edgemarkerlist;
#endif /* not ANSI_DECLARATORS */
...

{
  ...
  if (*edgelist == (int64_t *) NULL) {
    *edgelist = (int64_t *) trimalloc((int64_t) (m->edges * 2 * sizeof(int64_t)));
  }
  /* Allocate memory for edge markers if necessary. */
  if (!b->nobound && (*edgemarkerlist == (int64_t *) NULL)) {
    *edgemarkerlist = (int64_t *) trimalloc((int64_t) (m->edges * sizeof(int64_t)));
  }
  elist = *edgelist;
...
void writevoronoi(struct mesh *m, struct behavior *b, REAL **vpointlist,
                  REAL **vpointattriblist, int64_t **vpointmarkerlist,
                  int64_t **vedgelist, int64_t **vedgemarkerlist, REAL **vnormlist)
#else /* not ANSI_DECLARATORS */
...
int64_t **vpointmarkerlist;
int64_t **vedgelist;
int64_t **vedgemarkerlist;
REAL **vnormlist;
#endif /* not ANSI_DECLARATORS */
...
void writevoronoi(struct mesh *m, struct behavior *b, char *vnodefilename,
                  char *vedgefilename, int64_t argc, char **argv)
#else /* not ANSI_DECLARATORS */
...

{
  ...
  REAL circumcenter[2];
  REAL xi, eta;
  int64_t vnodenumber, vedgenumber;
  int64_t p1, p2;
  int64_t i;
  triangle ptr;                         /* Temporary variable used by sym(). */
  ...
  if (*vpointlist == (REAL *) NULL) {
    *vpointlist = (REAL *) trimalloc((int64_t) (m->triangles.items * 2 *
                                            sizeof(REAL)));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == (REAL *) NULL) {
    *vpointattriblist = (REAL *) trimalloc((int64_t) (m->triangles.items *
                                                  m->nextras * sizeof(REAL)));
  }
  *vpointmarkerlist = (int64_t *) NULL;
  plist = *vpointlist;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    * (int64_t *) (triangleloop.tri + 6) = (int64_t) vnodenumber;
    triangleloop.tri = triangletraverse(m);
    vnodenumber++;
  }
  ...
  if (*vedgelist == (int64_t *) NULL) {
    *vedgelist = (int64_t *) trimalloc((int64_t) (m->edges * 2 * sizeof(int64_t)));
  }
  *vedgemarkerlist = (int64_t *) NULL;
  /* Allocate memory for output Voronoi norms if necessary. */
  if (*vnormlist == (REAL *) NULL) {
    *vnormlist = (REAL *) trimalloc((int64_t) (m->edges * 2 * sizeof(REAL)));
  }
  elist = *vedgelist;
  normlist = *vnormlist;
  coordindex = 0;
...
#ifdef ANSI_DECLARATORS
void writeneighbors(struct mesh *m, struct behavior *b, int64_t **neighborlist)
#else /* not ANSI_DECLARATORS */
void writeneighbors(m, b, neighborlist)
struct mesh *m;
struct behavior *b;
int64_t **neighborlist;
#endif /* not ANSI_DECLARATORS */
...

{
  ...
  struct otri triangleloop, trisym;
  int64_t elementnumber;
  int64_t neighbor1, neighbor2, neighbor3;
  triangle ptr;                         /* Temporary variable used by sym(). */
  ...
  if (*neighborlist == (int64_t *) NULL) {
    *neighborlist = (int64_t *) trimalloc((int64_t) (m->triangles.items * 3 *
                                             sizeof(int64_t)));
  }
  nlist = *neighborlist;
  index = 0;
  ...
  elementnumber = b->firstnumber;
  while (triangleloop.tri != (triangle *) NULL) {
    * (int64_t *) (triangleloop.tri + 6) = (int64_t) elementnumber;
    triangleloop.tri = triangletraverse(m);
    elementnumber++;
  }
  * (int64_t *) (m->dummytri + 6) = -1;
  ...
  while (triangleloop.tri != (triangle *) NULL) {
    ...
    sym(triangleloop, trisym);
    neighbor1 = * (int64_t *) (trisym.tri + 6);
    triangleloop.orient = 2;
    sym(triangleloop, trisym);
    neighbor2 = * (int64_t *) (trisym.tri + 6);
    triangleloop.orient = 0;
    sym(triangleloop, trisym);
    neighbor3 = * (int64_t *) (trisym.tri + 6);
#ifdef TRILIBRARY
...

{
  ...
  int64_t angletable[18];
  int64_t aspecttable[16];
  int64_t aspectindex;
  int64_t tendegree;
  int64_t acutebiggest;
  int64_t i, ii, j, k;
